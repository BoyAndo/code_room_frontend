...
/**
 * GET /api/chat/landlord-chats
 * Obtiene la lista de conversaciones del arrendador loggeado.
 */
export async function GET(request: Request) {
  try {
    const url = new URL(request.url);
    const landlordIdString = url.searchParams.get("landlordId");

    // 💡 DEBUG: Verificar el ID de entrada
    console.log(
      `DEBUG: 🔍 Buscando chats para Landlord ID (string): ${landlordIdString}`
    );

    if (!landlordIdString) {
      return NextResponse.json(
        { error: "Falta landlordId en la query." },
        { status: 400 }
      );
    }

    const landlordIdNumber = Number(landlordIdString);

    if (isNaN(landlordIdNumber)) {
      return NextResponse.json(
        { error: "El landlordId no es un número válido." },
        { status: 400 }
      );
    }

    // 2. Consulta a Supabase: Traer todos los mensajes donde el arrendador loggeado
    // es el remitente O el destinatario.
    const filterQuery = `sender_id.eq.${landlordIdNumber},recipient_id.eq.${landlordIdNumber}`;
    console.log(
    ...
    (messages as Message[]).forEach((msg) => {
      // 🛑 LÓGICA CLAVE CORREGIDA: Determinamos el ID del estudiante usando el campo de rol.
      // Así, evitamos la colisión de IDs.
      const studentId =
        msg.sender_role === "STUDENT" ? msg.sender_id : msg.recipient_id;

      // 3b. Crear la clave única de la conversación (Estudiante + Propiedad)
      const key = `${String(studentId)}-${String(msg.property_id)}`;
    ...
    console.error("DEBUG: 🛑 Error inesperado en GET /landlord-chats:", e);
    return NextResponse.json(
