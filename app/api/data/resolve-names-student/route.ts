// app/api/data/resolve-names-student/route.ts
...
/**
 * POST /api/data/resolve-names-student
 * Proxy que reenvía solicitudes al microservicio de registro para obtener nombres de arrendadores y propiedades.
 * Similar a resolve-names pero optimizado para estudiantes (obtiene datos de landlords).
 */
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as {
      propertyIds?: Array<number | string>;
      landlordIds?: Array<number | string>; // Estudiantes necesitan info de arrendadores
    };
    ...
    if (propertyIds.length === 0 && landlordIds.length === 0) {
    ...
    const response = await fetch(`${MICROSERVICE_DATA_API_URL}/resolve-names`, {
      ...
      body: JSON.stringify({
        ...
        landlordIds: landlordIds.map(String), // Solo necesitamos landlords para estudiantes
      }),
    });
